...
async def assign_permission_to_role(role_permission_data: RolePermissionCreate, db: AsyncSession = Depends(get_db)):
    try:
        ...
        new_role_permission = RolePermissions(**role_permission_data.model_dump())
...
async def get_roles_with_permission(permission_id: int, db: AsyncSession = Depends(get_db)):
    try:

...
async def get_entities_by_client(client_id: UUID, db: AsyncSession = Depends(get_db)):
    try:
...
async def update_entity(entity_id: UUID, entity_data: ClientEntityUpdate, db: AsyncSession = Depends(get_db)):
    try:
        result = await db.execute(select(ClientEntity).where(ClientEntity.EntityID == entity_id))
        entity = result.scalar_one_or_none()

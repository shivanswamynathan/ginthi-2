...
async def create_client(client_data: ClientCreate, db: AsyncSession = Depends(get_db)):
    try:
        ...
        result = await db.execute(select(Clients).where(Clients.ClientID == client_data.ClientID))
        existing = result.scalar_one_or_none()
        
        if existing:
            logger.warning(f"Duplicate client creation attempt: ClientID {client_data.ClientID}")
            raise HTTPException(
                status_code=400,
                detail=f"Client with ClientID {client_data.ClientID} already exists"
            )
        ...
        # Create new client
        new_client = Clients(**client_data.model_dump())

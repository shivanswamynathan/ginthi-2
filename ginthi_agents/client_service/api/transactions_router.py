...
async def create_transaction(transaction_data: TransactionCreate, db: AsyncSession = Depends(get_db)):
    try:
        ...
        result = await db.execute(select(VendorTransactions).where(VendorTransactions.TransactionID == transaction_data.TransactionID))
        existing = result.scalar_one_or_none()
        
        if existing:
            logger.warning(f"Duplicate transaction creation attempt: TransactionID {transaction_data.TransactionID}")
            raise HTTPException(
                status_code=400,
                detail=f"Transaction with TransactionID {transaction_data.TransactionID} already exists"
            )
        ...
        new_transaction = VendorTransactions(**transaction_data.model_dump())
...
async def get_transactions_by_vendor(vendor_id: int, db: AsyncSession = Depends(get_db)):
    try:
...
async def update_transaction(transaction_id: int, transaction_data: TransactionUpdate, db: AsyncSession = Depends(get_db)):
    try:
        result = await db.execute(select(VendorTransactions).where(VendorTransactions.TransactionID == transaction_id))
        transaction = result.scalar_one_or_none()

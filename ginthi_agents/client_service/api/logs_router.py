...
async def get_action_log(log_id: UUID, db: AsyncSession = Depends(get_db)):
    try:
...
async def get_transaction_log(log_id: UUID, db: AsyncSession = Depends(get_db)):
    try:
...
async def get_logs_by_transaction(transaction_id: UUID, db: AsyncSession = Depends(get_db)):
    try:
...
async def get_user_log(log_id: UUID, db: AsyncSession = Depends(get_db)):
    try:
...
async def get_logs_by_user(user_id: UUID, skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)):
    try:

...
async def create_vendor(vendor_data: VendorCreate, db: AsyncSession = Depends(get_db)):
    try:
        ...
        result = await db.execute(select(VendorMaster).where(VendorMaster.VendorID == vendor_data.VendorID))
        existing = result.scalar_one_or_none()
        
        if existing:
            logger.warning(f"Duplicate vendor creation attempt: VendorID {vendor_data.VendorID}")
            raise HTTPException(
                status_code=400,
                detail=f"Vendor with VendorID {vendor_data.VendorID} already exists"
            )
        ...
        # Create new vendor
        new_vendor = VendorMaster(**vendor_data.model_dump())

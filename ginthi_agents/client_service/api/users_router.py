...
async def create_user(user_data: UserCreate, db: AsyncSession = Depends(get_db)):
    try:
        ...
        result = await db.execute(select(Users).where(Users.UserID == user_data.UserID))
        existing = result.scalar_one_or_none()
        
        if existing:
            logger.warning(f"Duplicate user creation attempt: UserID {user_data.UserID}")
            raise HTTPException(
                status_code=400,
                detail=f"User with UserID {user_data.UserID} already exists"
            )

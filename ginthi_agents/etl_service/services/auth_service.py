...
class AuthService:
    ...
    async def login(page: Page) -> bool:
        try:
            ...
            await page.goto(AuthService.LOGIN_PAGE, wait_until="networkidle", timeout=60000)
            
            logger.info("Page loaded. Waiting for form...")
            await page.wait_for_selector('input[placeholder*="username"], input[placeholder*="Enter your username"]', timeout=60000)
            
            logger.info("Filling username...")
            await page.fill('input[placeholder*="username"]', settings.SUPPLYNOTE_USERNAME)
            logger.info(f"Username filled: {settings.SUPPLYNOTE_USERNAME}")
            
            await asyncio.sleep(1)
            
            logger.info("Filling password...")
            await page.fill('input[placeholder*="password"]', settings.SUPPLYNOTE_PASSWORD)
